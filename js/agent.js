...
3. 全部5类智能体、质检流水线、知识库处理流水线统一遵守本整套系统提示词，不得私自删减、放宽任意条款。`;
    ...
    const TOOLS = [
        ...
        {type:"function", function:{
            name:"create_tool",
            description:"自主创建新工具。当现有工具（知识库检索/舰船查询/战斗推演/联网搜索/提问）无法满足用户任务时调用，由你自己编写工具代码（async (args, emit) => 返回值格式），并提供工具名称与作用标注。创建后系统会自动进行语法编译检查与LLM逻辑审查，通过即可用；不通过会自动尝试修复。注意：你写的工具代码会被保存到系统（本地持久化），当前及后续对话都会持续可用、可随时再次调用，无需重复创建。",
            parameters:{type:"object", properties:{
