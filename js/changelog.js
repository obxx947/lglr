...
(function(){
    // ======== 项目更新记录（时间+内容，最新在上） ========
    const CHANGELOG = [
        {time:'2026-08-12', content:'📜 系统提示词全面升级（五轮迭代评测版）：①五轮评测打分体系（输出：能量/物理/高闪避/综合常规分项+极端专项；扛伤：4组标准输出场景+极端混合伤害专项）；②全局统一配队硬性规则（参考《数据1-5》《例子1-31》≥3种思路、文档可信度优先级、基础/半改×180%/满改×220%属性换算）；③新增舰船名称与数据核验（舰船名不明查黑话.txt/md，每艘加入舰船查舰船人口.txt/舰船基础信息.md 服役上限人口搭载）；④附加永久执行禁令；⑤工具调用上限 1000次/单工具100次（代码与提示词同步）。'},
        {time:'2026-08-12', content:'🧩 AI 思考/状态面板重做：AI思考固定顶部面板、AI状态固定底部面板（输入框上方）——均实时更新、内容自动滚动到底部、可折叠（收起后不被新状态顶开）、切换对话自动清空跟随当前对话；所有状态事件全量进入状态面板不再遗漏。'},
        {time:'2026-08-11', content:'📱 手机竖屏优化：☰ 会话抽屉（切换/新建/清空会话）、markdown 表格横向滚动、消息区/输入区窄屏适配、iPhone 底部安全区；局域网部署 http://IP:3002。'},
        {time:'2026-08-11', content:'🧬 全智能体统一 system_prompt：子代理/QA质检流水线/kb-dev 知识库开发全部复用 agent.js 唯一共享提示词（data/system_prompt.md 单一主源，改一处全部生效）；qa.js 5处与 kb-dev 2处 system 层已接入。'},
        {time:'2026-08-09', content:'🛂 计划审批操作栏上线：AI 输出【本次任务完整执行计划书】后，聊天区自动弹出「需要权限·实施计划」操作栏（与 AI 提问卡片同款金色样式）——点击「批准」即以"1"发送批准并开始实施；直接打字则作为修改意见，AI 重制计划书循环等待批准。'},
        {time:'2026-08-09', content:'📚 新增第二知识库：备份旧资料（51文件：舰船资料txt/精炼数据）作为补充库，第一知识库查不到/不清晰时自动去第二知识库查找；两库合并检索（主库47md + 备份51文件）。'},
        {time:'2026-08-09', content:'🛂 核心强制总规则上线：任何执行类任务先输出完整计划书（任务目标/分步执行/操作顺序/注意事项/风险点/调用工具/验收标准）并附批准/修改交互，获批后才执行；闲聊自动休眠。'},
        {time:'2026-08-09', content:'🔒 新增规则：加入新舰船（含舰载机）必须向用户提问附数据经同意；配置思路参考数据01-05讲解；禁止用"火力总览"推导输出（按战斗机制.md计算）；护航机制（输出队只拼输出/抗伤队只拼存活/护航舰未灭被护航舰不受伤害）；优先舰船清单（VB/星脉/索姆河/海氏/风暴/大剑/游骑兵离子/泡泡龙/猎兵/刺水母/狩猎/太阳鲸）。'},
